...
```bash
git clone https://github.com/priesouza/BOSL_Relative_Sensitivity.git
cd BOSL_Relative_Sensitivity
```
...
2. Edit the parameters at the top of the script (lines 1–30) to match your data and settings:
...
A single-aliquot regenerative-dose (or equivalent) `.binx` file (Risø reader format), read via `Luminescence::read_BIN2R()`.
